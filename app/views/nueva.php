                        ...
                        <div class="widget-list mb-10">
                            ...
                            <div class="sidebar-body">
                                <ul class="sidebar-list">
                                    <li><a href="nueva.php<?= $sort || $perPage || $size ? '?' . http_build_query(array_filter(['sort' => $sort, 'perPage' => $perPage, 'size' => $size])) : '' ?>"<?= $categoryId === null ? ' class="active text-danger"' : '' ?>>Todas</a></li>
                                    <?php foreach ($categories as $cat): ?>
                                        <li><a href="nueva.php?<?= http_build_query(array_filter(['category' => $cat['id'], 'sort' => $sort, 'perPage' => $perPage, 'size' => $size])) ?>"<?= $categoryId === (int)$cat['id'] ? ' class="active text-danger"' : '' ?>><?= htmlspecialchars($cat['name'], ENT_QUOTES, 'UTF-8'); ?> (<?= $cat['usage_count']; ?>)</a></li>
                                    <?php endforeach; ?>
                                </ul>
                            </div>
                        </div>
